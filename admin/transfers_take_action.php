<?php
session_start();
$error_page = "../../abanfo/in/examples/_1transfers_view_transfers.php";
...
***********************************************************************************************************************/
	...
	. USER_BIO_TABLE_NAME . ".investor_id="  
	. LOGIN_TABLE_NAME . ".id "
	. " WHERE " . SHARES_TRANSFER_TABLE_NAME . ".admin_review_status = 0 AND " . SHARES_TRANSFER_TABLE_NAME . ".admin_id = '' AND " . LOGIN_TABLE_NAME . ".flag = 0 AND " . SHARES_TRANSFER_TABLE_NAME . ".sku = ?";
	...
	$prepared_statement_results_array->bind_result($fcm_token, $fcm_token_ios, $debit_wallet_usd, $user_language, $country, $sender_id, $share_id, $receiver_id, $receiver_share_id, $num_shares_transfered, $shares_parent_name, $transfer_type, $transfer_sku);

	$prepared_statement_results_array->fetch();

	if(trim($sender_id) == "" || trim($share_id) == "" || trim($receiver_id) == "" || trim($receiver_share_id) == "" || $num_shares_transfered <= 0){
		$miscellaneousObject->respondFrontEnd2("black", $error_page, $languagesObject->getLanguageString("something_went_wrong", $input_language));
	}
    ...
    if($prepared_statement_2 !== false){
        ...
        if($prepared_statement_results_array_2 !== false && $prepared_statement_results_array_2[0] != "shares_parent_id"){
	    	$new_transfer_exists = 1;
        } else {
	    	$new_transfer_exists = 0;
        }
    } else {
	...
	if($var_action_type == 1){
	    ...
	    if($new_transfer_exists == 0){
			...
			if($prepared_statement === false){
				$miscellaneousObject->respondFrontEnd2("black", $error_page, "[NT]- Failed to unflag shares for receiver. If this continues, inform Super Admin");
			}

		}

		$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, DEVELOPER_USING_ADMIN_LIVE_MODE), "UPDATE " . SHARES_TRANSFER_TABLE_NAME . " SET admin_review_status = 1, admin_id = ?, review_datetime = ? WHERE sku = ?", 3, "ssi", array($var_admin_id, date("Y-m-d H:i:s"), $var_sku));

		if($prepared_statement === false){
			$miscellaneousObject->respondFrontEnd2("black", $error_page, "[NT]- Failed to update transfer review status. Inform Super Admin");
		}

		$sys_status_info = $languagesObject->getLanguageString("transfer_confirmed", $user_language);
		$sys_status_info2 = $num_shares_transfered . " " . $shares_parent_name . " " . $languagesObject->getLanguageString("shares_transfer_confirmed", $user_language);
	...
	if($sender_shares_total_yield <= 0 || $sender_num_of_shares <= 0){
		...
		$new_sender_shares_total_yield = $sender_shares_total_yield + (($sender_shares_total_yield/$sender_num_of_shares) * $num_shares_transfered);
	}
	...
	if($receiver_shares_total_yield <= 0 || $receiver_num_of_shares <= 0){
		...
		$new_receiver_shares_total_yield = $receiver_shares_total_yield - (($receiver_shares_total_yield/$receiver_num_of_shares) * $num_shares_transfered);
	}




		if($transfer_type != "sale" && $transfer_type != "coupon"){

			if($new_transfer_exists == 0){
				$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, DEVELOPER_USING_ADMIN_LIVE_MODE), "UPDATE " . SHARES_OWNED_BY_INVESTOR_TABLE_NAME . " SET flag = 0, num_of_shares = num_of_shares - $num_shares_transfered, admin_review_status = 1, total_yield = ? WHERE share_id = ?", 2, "ds", array($new_receiver_shares_total_yield, $receiver_share_id));

				if($prepared_statement === false){
					$miscellaneousObject->respondFrontEnd2("black", $error_page, "[NT]- Failed to remove shares from receiver. If this continues, inform Super Admin");
				}

				$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, DEVELOPER_USING_ADMIN_LIVE_MODE), "UPDATE " . SHARES_OWNED_BY_INVESTOR_TABLE_NAME . " SET num_of_shares = num_of_shares + $num_shares_transfered, total_yield = ? WHERE share_id = ?", 2, "ds", array($new_sender_shares_total_yield, $share_id));

				if($prepared_statement === false){
					$miscellaneousObject->respondFrontEnd2("black", $error_page, "[NT]- Failed to add shares to sender. If this continues, inform Super Admin");
				}
			} else {
				$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, DEVELOPER_USING_ADMIN_LIVE_MODE), "UPDATE " . SHARES_OWNED_BY_INVESTOR_TABLE_NAME . " SET num_of_shares = num_of_shares - $num_shares_transfered, total_yield = ? WHERE share_id = ?", 2, "ds", array($new_receiver_shares_total_yield, $receiver_share_id));
		...
		$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, DEVELOPER_USING_ADMIN_LIVE_MODE), "UPDATE " . SHARES_TRANSFER_TABLE_NAME . " SET admin_review_status = 2, admin_id = ?, review_datetime = ? WHERE sku = ?", 3, "ssi", array($var_admin_id, date("Y-m-d H:i:s"), $var_sku));

		if($prepared_statement === false){
			$miscellaneousObject->respondFrontEnd2("black", $error_page, "[NT]- Failed to update transfer review status. Inform Super Admin");
		}

		$sys_status_info = $languagesObject->getLanguageString("transfer_failed", $user_language);
		$sys_status_info2 = $num_shares_transfered . " " . $shares_parent_name . " " . $languagesObject->getLanguageString("shares_transfer_failed_to_confirm", $user_language);
