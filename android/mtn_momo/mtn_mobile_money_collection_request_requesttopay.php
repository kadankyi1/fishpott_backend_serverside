...
if( $_SERVER["REQUEST_METHOD"] == "POST" &&
	...
	isset($_POST["mtn_phone_number"]) && trim($_POST["mtn_phone_number"]) != "" &&
	isset($_POST["amount"]) && trim($_POST["amount"]) != "" && floatval($_POST["amount"]) > 0 &&
	...
	$input_mtn_phone_number = trim($_POST["mtn_phone_number"]);
...
***********************************************************************************************************/
	...
	if($input_my_currency == "USD" || $input_my_currency == "$"){
		//$input_amount = $USD_GHS * $input_amount
		$input_my_currency = "USD";
	} else if($input_my_currency == "GBP" || $input_my_currency == "£"){
		//$input_amount = $GBP_GHS * $input_amount
		$input_my_currency = "GBP";
	} else if($input_my_currency == "Ghc" || $input_my_currency == "GHS" || $input_my_currency == "₵"){
		$input_my_currency = "GHS";
	}

	// SENDING MOMO COLLECTION REQUEST

	$sys_callback_url = "";

	$sys_mtn_api_user = $mtnMomoObject->mtnCreateApiUser($miscellaneousObject->gen_uuid(), MTN_MOMO_COLLECTION_API_KEY_1, MTN_CREATE_API_USER_URL, $sys_callback_url);

	//echo "\n\n\n sys_mtn_api_user : " . $sys_mtn_api_user . "\n";
	...
	$sys_mtn_api_key = $mtnMomoObject->mtnGetApiKey($sys_mtn_api_user, MTN_MOMO_COLLECTION_API_KEY_1, $api_url);

	//echo "\n\n\n sys_mtn_api_key : " . $sys_mtn_api_key;

	if($sys_mtn_api_key == "0"){
		$miscellaneousObject->respondFrontEnd3(3, $languagesObject->getLanguageString("request_failed", $input_language));
	}

	$sys_mtn_api_token = $mtnMomoObject->mtnGetApiToken($sys_mtn_api_user, $sys_mtn_api_key, MTN_MOMO_COLLECTION_API_KEY_1, MTN_CREATE_COLLECTION_TOKEN_URL);

	//echo "\n\n\n sys_mtn_api_token : " . $sys_mtn_api_token;

	if($sys_mtn_api_token == "0"){
		$miscellaneousObject->respondFrontEnd3(3, $languagesObject->getLanguageString("request_failed", $input_language));
	}

	$sys_callback_url = HTTP_HEAD_FOR_FISHPOTT . "/inc/android/mtn_momo/mtn_mobile_money_collection_request_response.php?phone=" . $input_phone . "&pn=" . $input_mypottname . "&reqphone=" . $input_mtn_phone_number . "&curr=" . $input_my_currency . "&amt=" . $input_amount . "&datetime=" . date("Y-m-d_H:i:s") . "&lang=" . $input_language;

	$sys_payer_message = "FishPott Wallet Credit of " . $input_my_currency .  $input_amount . " for @" . $input_mypottname;

	//echo "\n\n\n sys_callback_url : " . $sys_callback_url . " \n\n\n" ;
	/*
	echo "\n\n\n input_amount : " . $input_amount;
	echo "\n\n\n input_my_currency : " . $input_my_currency;
	echo "\n\n\n input_mtn_phone_number : " . $input_mtn_phone_number;
	echo "\n\n\n sys_payer_message : " . $sys_payer_message;
	*/

	$sys_mtn_requesttopay_response = $mtnMomoObject->mtnSendMomoCollectionRequest($sys_mtn_api_token, $sys_callback_url, MTN_CURRENT_TARGET_ENVIRONMENT, $miscellaneousObject->gen_uuid(), MTN_MOMO_COLLECTION_API_KEY_1,  MTN_CREATE_COLLECTION_REQUESTTOPAY_URL, $input_amount, "EUR", $input_mypottname, "MSISDN", $input_mtn_phone_number, $sys_payer_message);

	//echo "\n\n\n sys_mtn_requesttopay_response : " . $sys_mtn_requesttopay_response; exit;
	...
	$sysResponse["data_returned"][0] = array(
		'1' => 1, 
		'2' => $languagesObject->getLanguageString("payment_request_has_been_sent_to_the_phone_number", $input_language),  
