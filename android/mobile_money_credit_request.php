...
if( $_SERVER["REQUEST_METHOD"] == "POST" &&
	...
	isset($_POST["pay_type"]) && trim($_POST["pay_type"]) != "" &&
	isset($_POST["transaction_id"]) && trim($_POST["transaction_id"]) != "" &&
	isset($_POST["amount_sent"]) && trim($_POST["amount_sent"]) != "" &&
	isset($_POST["sender_name"]) && trim($_POST["sender_name"]) != "" &&
	isset($_POST["send_date"]) && trim($_POST["send_date"]) != "" &&
	...
	$input_amount_sent = floatval($_POST["amount_sent"]);
	$input_sender_name = trim($_POST["sender_name"]);
	$input_send_date = trim($_POST["send_date"]);
...
*/
	...
	$sys_status_info = $languagesObject->getLanguageString("credit_request_sent_for", $input_language) . " " . $sys_currency_symbol . $input_amount_sent . ". " . $languagesObject->getLanguageString("you_will_be_notified_when_the_request_is_processed", $input_language);
	...
	$sysResponse["data_returned"][0]  = array(
	 ...
	 //echo "here 999 \n";
	$miscellaneousObject->sendEmail(SENDING_EMAIL_ANDROID, "FishPott Android App", RECEIVING_EMAIL_ANDROID, "CREDIT REQUEST CONFIRMATION NEEDEDED", "WITHDRAWAL REQUEST CONFIRMATION NEEDEDED");	
