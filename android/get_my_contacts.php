	...
	if(isset($_SESSION["all_contacts_numbers"]) && trim($_SESSION["all_contacts_numbers"]) != "" && count($input_all_contacts_numbers_array) > 1) {
        ...
        if(trim($query_valuestype_string) != ""){
		    ...
		    while($prepared_statement_results_array->fetch()){
				...
				$chat_id = "s_" . $input_id . "_r_" . $investor_id;
...
// FETCHING MORE LINKUPS IF WE GOT LESS THAN 100 FROM CONTACTS
if($count < 100) {
	...
	. ' WHERE ' . LOGIN_TABLE_NAME . '.flag = 0  AND ' . USER_BIO_TABLE_NAME . '.investor_id != ?  AND ' . LINKUPS_TABLE_NAME . '.receiver_id = ? LIMIT 100';

	$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), $contacts_query, 2, "ss", array($input_id, $input_id));
	...
	// GETTING RESULTS OF QUERY INTO AN ARRAY
	$prepared_statement_results_array = $preparedStatementObject->getPreparedStatementQueryResults($prepared_statement, array("first_name", "last_name", "pot_name", "phone", "profile_picture", "verified_tag", "investor_id", "net_worth", "investing_points", "total_amount_made_onfp"), 11, 2);
	//BINDING THE RESULTS TO VARIABLES
