...
if( $_SERVER["REQUEST_METHOD"] == "POST" &&
	...
	isset($_POST["last_sku"]) && intval($_POST["last_sku"]) > -1 &&
	...
	$input_chat_id = trim($_POST["chat_id"]);
	$input_my_last_sku = intval($_POST["last_sku"]);
...
***********************************************************************************************************/
	...
	$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "UPDATE " . USER_BIO_TABLE_NAME . " SET coins_secure_datetime = ? WHERE investor_id = ?", 2, "ss", array(date("Y-m-d H:i:s"), $input_id));
		...
		$news_fetch_query =  "SELECT  sku, sender_pottname, receiver_pottname, message_text, message_time, chat_id FROM " . CHAT_MESSAGES_TABLE_NAME . " WHERE ((sender_pottname = ? AND receiver_pottname = ?) OR (receiver_pottname = ? AND sender_pottname = ?))  AND sku > ? ORDER BY  " . CHAT_MESSAGES_TABLE_NAME . ".sku ASC";


	$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), $news_fetch_query, 5, "ssssi",array($input_mypottname, $input_receiver_pottname, $input_mypottname, $input_receiver_pottname, $input_my_last_sku));
