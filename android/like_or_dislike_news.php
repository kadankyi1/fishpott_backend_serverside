...
if( $_SERVER["REQUEST_METHOD"] == "POST" &&
	...
	isset($_POST["like_type"]) && trim($_POST["like_type"]) != "" &&
	isset($_POST["news_id"]) && trim($_POST["news_id"]) != "" &&
	...
	$input_like_type = intval($_POST["like_type"]);
...
***********************************************************************************************************/
	...
	. USER_BIO_TABLE_NAME . ".investor_id="  
	. LOGIN_TABLE_NAME . ".id "
	. " WHERE " . NEWS_TABLE_NAME . ".flag = 0 AND " . LOGIN_TABLE_NAME . ".flag = 0 AND " . NEWS_TABLE_NAME . ".news_id = ?";


	$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), $news_fetch_query, 1, "s",array($input_news_id));
	...
	$prepared_statement_results_array->bind_result($fcm_token, $fcm_token_ios, $sys_inputtor_pearls, $language, $country, $investor_id);
	$prepared_statement_results_array->fetch();
	$input_newsmaker_id = $investor_id;
	if(trim($language) == ""){
		$language = $country;
	}
	...
	$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "SELECT like_type, liker_investor_id FROM " . LIKES_TABLE_NAME . " WHERE likes_news_id = ? AND liker_investor_id = ?", 2, "ss", array($input_news_id, $input_id));
	...
	// GETTING RESULTS OF QUERY INTO AN ARRAY
	$prepared_statement_results_array = $preparedStatementObject->getPreparedStatementQueryResults($prepared_statement, array("like_type", "liker_investor_id"), 2, 1);
	...
	// IF THE DATABASE QUERY GOT NO RESULTS
	if($prepared_statement_results_array[1] == "liker_investor_id"){
		//$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "DELETE FROM " . LIKES_TABLE_NAME . " WHERE likes_news_id = ? AND liker_investor_id = ?", 2, "ss", array($input_news_id, $input_id));
		if($input_like_type != 1 && $input_like_type != 0){
			$miscellaneousObject->respondFrontEnd3(3, $languagesObject->getLanguageString("request_failed", $input_language));
		}
		if($input_like_type == 0){
			$sys_inputtor_pearls = $sys_inputtor_pearls - 1;
		} else if( $input_like_type == 1){
			$sys_inputtor_pearls = $sys_inputtor_pearls + 1;
		}

		$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "INSERT INTO " . LIKES_TABLE_NAME . " (likes_news_id, liker_investor_id, like_type, date_time) VALUES (?, ?, ?, ?)" , 4, "ssis", array($input_news_id, $input_id, $input_like_type, date("Y-m-d H:i:s")));
		...
		if($prepared_statement_results_array[0] == 1){
			if($input_like_type == 0 && $input_like_type != 1){
				$sys_inputtor_pearls = $sys_inputtor_pearls - 2;
			} else if($input_like_type != 0 && $input_like_type != 1){
				$sys_inputtor_pearls = $sys_inputtor_pearls - 1;
			}
		...
		if($input_like_type != 1 && $input_like_type != 0){
			...
			$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "UPDATE " . LIKES_TABLE_NAME . " SET like_type = ? WHERE likes_news_id = ? AND liker_investor_id = ?", 3, "iss", array($input_like_type, $input_news_id, $input_id));
			if($prepared_statement === false){
				$miscellaneousObject->respondFrontEnd3(3, $languagesObject->getLanguageString("request_failed", $input_language));
			}
		}
	...
	$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "UPDATE " . USER_BIO_TABLE_NAME . " SET net_worth = ? WHERE investor_id = ?", 2, "is", array($sys_inputtor_pearls, $input_newsmaker_id));
	if($prepared_statement === false){
		$miscellaneousObject->respondFrontEnd3(3, $languagesObject->getLanguageString("request_failed", $input_language));
	}


	if($input_like_type == 1){
		$send_notification = true;
		$sys_status_info = "@" . $input_mypottname . " " . $languagesObject->getLanguageString("likes_your_post", $language);
		$sys_status_info2 = $languagesObject->getLanguageString("post_liked", $language);
		$not_type = "like";
	} else if($input_like_type == 0){
		$send_notification = true;
		$sys_status_info = "@" . $input_mypottname . " " . $languagesObject->getLanguageString("dislikes_your_post", $language);
		$sys_status_info2 = $languagesObject->getLanguageString("post_disliked", $language);
		$not_type = "dislike";
	} else {
		$send_notification = false;
	}

	if(trim($input_newsmaker_id) == $input_id){
		$send_notification = false;
	}
