...
if( $_SERVER["REQUEST_METHOD"] == "POST" &&
	...
	isset($_POST["stats_type"]) && trim($_POST["stats_type"]) != "" && intval($_POST["stats_type"]) > 199 &&
	isset($_POST["shows_news"]) && trim($_POST["shows_news"]) != "" &&
	...
	$input_stats_type = intval($_POST["stats_type"]);
	$input_shows_news = intval($_POST["shows_news"]);
...
***********************************************************************************************************/
	...
	if($input_shows_news == 1){
	      ...
	      . " WHERE " . NEWS_TABLE_NAME . ".flag = 0 AND  " . NEWS_TABLE_NAME . ".news_id = ? ";

		$value_type_string = "s";
		$value_array = array($input_news_id);
	...
	//REFORMATING CONSTANTS FOR STATS TYPE
	$input_stats_type_real = $input_stats_type;
	if($input_stats_type_real == 203){
		// likes
		$input_stats_type = 0;
	} else if($input_stats_type_real == 201){
		// dislikes
		$input_stats_type = 1;
	} else if($input_stats_type_real == 213){
		// views
		$input_stats_type = 2;
	} else if($input_stats_type_real == 214){
		// purchases
		$input_stats_type = 3;
	...
	if($input_stats_type == 0 || $input_stats_type == 1){
		if($input_stats_type == 1){
			$sys_reaction_type = 0;
		} else {
			$sys_reaction_type = 1;
		}
		...
		if($input_lastest_sku > 0){
			$sku_query_addition = " AND " . LIKES_TABLE_NAME . ".sku < ? ";
			$value_type_string = "i";
		...
		. USER_BIO_TABLE_NAME . ".pot_name,  " 
		. USER_BIO_TABLE_NAME . ".profile_picture,  " 
		. LIKES_TABLE_NAME . ".date_time,  " 
		. LIKES_TABLE_NAME . ".like_type,  " 
		. LIKES_TABLE_NAME . ".sku FROM "  
		. USER_BIO_TABLE_NAME . " INNER JOIN "
		. LIKES_TABLE_NAME . " ON  "
		. USER_BIO_TABLE_NAME . ".investor_id=" 
		. LIKES_TABLE_NAME . ".liker_investor_id "
		. " WHERE " . LIKES_TABLE_NAME . ".likes_news_id = ?  AND " . LIKES_TABLE_NAME . ".like_type = $sys_reaction_type " . $sku_query_addition . " ORDER BY " . LIKES_TABLE_NAME .".sku DESC  LIMIT 150";

	} else if($input_stats_type == 2){

		if($input_lastest_sku > 0){
			$sku_query_addition = " AND " . USER_BIO_TABLE_NAME . ".sku < ? ";
			$value_type_string = "i";
			$value_array = array($input_lastest_sku);
		}
		...
		. USER_BIO_TABLE_NAME . ".pot_name,  " 
		. USER_BIO_TABLE_NAME . ".profile_picture,  " 
		. VIEWS_TABLE_NAME . ".date_time,  " 
		. VIEWS_TABLE_NAME . ".views,  " 
		. VIEWS_TABLE_NAME . ".sku FROM "  
		. USER_BIO_TABLE_NAME . " INNER JOIN "
		. VIEWS_TABLE_NAME . " ON  "
		. USER_BIO_TABLE_NAME . ".investor_id=" 
		. VIEWS_TABLE_NAME . ".viewer_id "
		. " WHERE " . VIEWS_TABLE_NAME . ".news_id = ?  " . $sku_query_addition . "  ORDER BY " . VIEWS_TABLE_NAME .".sku DESC  LIMIT 150";

	} else if($input_stats_type == 3){

		if($input_lastest_sku > 0){
			$sku_query_addition = " AND " . PURCHASES_TABLE_NAME . ".sku < ? ";
			$value_type_string = "i";
			$value_array = array($input_lastest_sku);
		}
		...
		. PURCHASES_TABLE_NAME . ".sku FROM "  
		. USER_BIO_TABLE_NAME . " INNER JOIN "
		. PURCHASES_TABLE_NAME . " ON  "
		. USER_BIO_TABLE_NAME . ".investor_id=" 
		. PURCHASES_TABLE_NAME . ".buyer_id "
		. " WHERE " . PURCHASES_TABLE_NAME . ".adetor_news_id = ?  " . $sku_query_addition . "  ORDER BY " . PURCHASES_TABLE_NAME .".sku DESC  LIMIT 150";

	} else {

		if($input_lastest_sku > 0){
			$sku_query_addition = " AND " . COMMENTS_TABLE_NAME . ".sku < ? ";
			$value_type_string = "si";
			$value_array = array($input_lastest_sku);
		}
		...
		. USER_BIO_TABLE_NAME . ".pot_name,  " 
		. USER_BIO_TABLE_NAME . ".profile_picture,  " 
		. COMMENTS_TABLE_NAME . ".date_time,  " 
		. COMMENTS_TABLE_NAME . ".comment,  " 
		. COMMENTS_TABLE_NAME . ".sku FROM "  
		. USER_BIO_TABLE_NAME . " INNER JOIN "
		. COMMENTS_TABLE_NAME . " ON  "
		. USER_BIO_TABLE_NAME . ".investor_id=" 
		. COMMENTS_TABLE_NAME . ".inputtor_id "
		. " WHERE " . COMMENTS_TABLE_NAME . ".news_id = ? " . $sku_query_addition . " ORDER BY " . COMMENTS_TABLE_NAME .".sku DESC  LIMIT 150";

	}

	if($input_lastest_sku > 0){
		$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), $news_fetch_query, 2, "si",array($input_news_id, $input_lastest_sku));
	} else {
	...
	if($input_stats_type == 0 || $input_stats_type == 1){
		...
		$prepared_statement_results_array = $preparedStatementObject->getPreparedStatementQueryResults($prepared_statement, array(
			...
			USER_BIO_TABLE_NAME . ".profile_picture", 
			VIEWS_TABLE_NAME . ".date_time", 
			VIEWS_TABLE_NAME . ".item_quantity", 
			VIEWS_TABLE_NAME . ".sku"
		), 5, 2);
	} else if($input_stats_type == 4){
		$prepared_statement_results_array = $preparedStatementObject->getPreparedStatementQueryResults($prepared_statement, array(
			USER_BIO_TABLE_NAME . ".pot_name", 
			USER_BIO_TABLE_NAME . ".profile_picture", 
			COMMENTS_TABLE_NAME . ".date_time", 
			COMMENTS_TABLE_NAME . ".comment", 
			COMMENTS_TABLE_NAME . ".sku"
		), 5, 2);
	}

	//BINDING THE RESULTS TO VARIABLES
	$prepared_statement_results_array->bind_result($pot_name, $profile_picture, $date_time, $sys_reaction_type, $sku);

	while($prepared_statement_results_array->fetch()){

			$newsTime = $timeObject->reformatDate("M j, y", $date_time);
		...
		$next  = array(
			"0a" => $sku,
			"1" => $input_stats_type_real, 
			"2" => $input_news_id,
			"3" => strval($sys_reaction_type), 
			"4" => $newsTime,  
			"5" => "", 
			"6" => "", 
			"7" => "", 
