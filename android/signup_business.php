...
if( $_SERVER["REQUEST_METHOD"] == "POST" &&
	isset($_POST["business_name"]) && trim($_POST["business_name"]) != "" &&
	isset($_POST["branch_location"]) && trim($_POST["branch_location"]) != "" &&
	...
	$input_first_name = trim($_POST["business_name"]);
	$input_last_name = trim($_POST["branch_location"]);
	...
	if( 
		...
		$validatorObject->stringIsNotMoreThanMaxLength($input_gender, 8) === false || 
	...
	if($input_pott_name == "mylinkups" || $input_referrer_pott_name == "mylinkups"){
	...
	if(($input_gender != "business") || $validatorObject->inputContainsOnlyAlphabetsWithUnderscore($input_language, false, "") ===  false){
		$miscellaneousObject->respondFrontEnd1($languagesObject->getLanguageString("error", $input_language), $languagesObject->getLanguageString("registration_failed", $input_language));
	}
	...
	//MASKING THE PHONE NUMBER AND POTT NAME IF PHONE NUMBER VERIFICATION IS ON
