...
***********************************************************************************************************/
	...
	$view_id = $input_news_id . "_" . $input_id;

	$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "SELECT views FROM " . VIEWS_TABLE_NAME . " WHERE view_id = ?", 1, "s", array($view_id));
	...
	// GETTING RESULTS OF QUERY INTO AN ARRAY
	$prepared_statement_results_array = $preparedStatementObject->getPreparedStatementQueryResults($prepared_statement, array("views"), 1, 1);
	...
	// IF THE DATABASE QUERY GOT NO RESULTS
	if($prepared_statement_results_array[0] == "views"){
		$views = 1;
		$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "INSERT INTO " . VIEWS_TABLE_NAME . " (view_id, news_id, viewer_id, views, date_time) VALUES (?, ?, ?, ?, ?)" , 5, "sssis", array($view_id, $input_news_id, $input_id, $views, date("Y-m-d H:i:s")));

		if($prepared_statement === false){
			$miscellaneousObject->respondFrontEnd3(3, $languagesObject->getLanguageString("request_failed", $input_language));
		}
	} else {
		$views = $prepared_statement_results_array[0] + 1;
		$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "UPDATE " . VIEWS_TABLE_NAME . " SET views = ?, date_time = ? WHERE view_id = ?", 3, "iss", array($views, date("Y-m-d H:i:s"), $view_id));
