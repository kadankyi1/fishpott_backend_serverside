...
if( $_SERVER["REQUEST_METHOD"] == "POST" &&
	...
	isset($_POST["verification_code"]) && trim($_POST["verification_code"]) != "" &&
	...
	$input_verification_code = trim($_POST["verification_code"]);
	...
	// MAKING SURE THE PHONE NUMBER AND RESET CODE CONFORM TO MAXLENGTH
	if($validatorObject->stringIsNotMoreThanMaxLength($input_phone, 15) || $validatorObject->stringIsNotMoreThanMaxLength($input_verification_code, 10)){
		$miscellaneousObject->respondFrontEnd1($languagesObject->getLanguageString("error", $input_language), $languagesObject->getLanguageString("verification_failed", $input_language));
	}

	//MAKING SURE THAT PHONE NUMBER AND THE RESET CODE CONTAINS NO TAGS
	if($validatorObject->stringContainsNoTags($input_phone) !== true || $validatorObject->stringContainsNoTags($input_verification_code) !== true){
		$miscellaneousObject->respondFrontEnd1($languagesObject->getLanguageString("error", $input_language), $languagesObject->getLanguageString("verification_failed", $input_language));
	}
	...
	$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "SELECT number_verifcation_date, number_verification_code, flag FROM " . LOGIN_TABLE_NAME . " WHERE number_login = ?", 1, "s", array($input_phone));
	if($prepared_statement === false){
		$miscellaneousObject->respondFrontEnd1($languagesObject->getLanguageString("error", $input_language), $languagesObject->getLanguageString("verification_failed", $input_language));
	}
	// GETTING RESULTS OF QUERY INTO AN ARRAY
	$prepared_statement_results_array = $preparedStatementObject->getPreparedStatementQueryResults($prepared_statement, array("number_verifcation_date", "number_verification_code", "flag"), 3, 1);

	if($prepared_statement_results_array === false){
		$miscellaneousObject->respondFrontEnd1($languagesObject->getLanguageString("error", $input_language), $languagesObject->getLanguageString("verification_failed", $input_language));
	}
	// IF THE DATABASE QUERY GOT NO RESULTS
	if(trim($prepared_statement_results_array[0]) == "number_verifcation_date" || trim($prepared_statement_results_array[1]) == "number_verification_code"){
		$miscellaneousObject->respondFrontEnd1($languagesObject->getLanguageString("error", $input_language), $languagesObject->getLanguageString("verification_failed", $input_language));
	}
...
*/
	...
	if($prepared_statement_results_array[1] == $input_verification_code){
		// UPDATING THE RESET DATE, THE PASSWORD
		$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "UPDATE " . LOGIN_TABLE_NAME . " SET number_verifcation_date = ?,  number_verification_code = '',  number_verified = ? , flag = 0 WHERE number_login = ?", 3, "sis", array(date("Y-m-d H:i:s"), 1, $input_phone));
		...
		$miscellaneousObject->respondFrontEnd1("yes", $languagesObject->getLanguageString("phone_number_verified_successfully", $input_language));
	} else if($prepared_statement_results_array[1] != $input_verification_code){
		// CLOSE DATABASE CONNECTION
		$dbObject->closeDatabaseConnection($prepared_statement);
		$miscellaneousObject->respondFrontEnd1($languagesObject->getLanguageString("error", $input_language), $languagesObject->getLanguageString("verification_failed", $input_language));
	} else {
