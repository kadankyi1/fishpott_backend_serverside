...
if( $_SERVER["REQUEST_METHOD"] == "POST" &&
	...
	isset($_POST["current_password"]) && trim($_POST["current_password"]) != "" &&
	...
	$input_myrawpass = trim($_POST["current_password"]);
	...
	$input_myrawpass_hashed = $validatorObject->hashString($input_myrawpass);

	if($input_myrawpass_hashed != $prepared_statement_results_array[0]){
...
***********************************************************************************************************/
	...
	$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "SELECT fcm_token, fcm_token_ios FROM " . USER_BIO_TABLE_NAME . " WHERE pot_name = ?", 1, "s", array($input_mypottname));
	if($prepared_statement === false){
		$miscellaneousObject->respondFrontEnd3(3, $languagesObject->getLanguageString("request_failed", $input_language));
	}
	...
	$sys_receiver_fcm_token = $prepared_statement_results_array[0];
	$sys_receiver_fcm_token_ios = $prepared_statement_results_array[1];
	...
	$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "UPDATE " . LOGIN_TABLE_NAME . " SET password = ? WHERE id = ?", 2, "ss", array($input_new_password_hashed, $input_id));

	if($prepared_statement === false){
		$miscellaneousObject->respondFrontEnd3(3, $languagesObject->getLanguageString("request_failed", $input_language));
	}
	
	$sys_status_info = $languagesObject->getLanguageString("your_password_has_been_changed_successfully", $input_language);
	...
	$miscellaneousObject->sendNotificationToUser(
		...
		$input_id, 
		$input_mypottname, 
		$languagesObject->getLanguageString("password_changed", $input_language), 
		$sys_status_info, 
		date("F j, Y"), 
		""
	);
