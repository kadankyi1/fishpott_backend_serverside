...
if( $_SERVER["REQUEST_METHOD"] == "POST" &&
	...
	isset($_POST["chat_id"]) && trim($_POST["chat_id"]) != "" &&
	...
	$input_chat_ids = trim($_POST["chat_id"]);
	...
	if(isset($_POST["message_time"]) && trim($_POST["message_time"]) != ""){
		$input_msg_datetime = $_POST["message_time"];
		$input_msg_datetime = $timeObject->reformatDate("Y-m-d H:i:s", $input_msg_datetime);
	} else {
		$input_msg_datetime = date("Y-m-d H:i:s");
	}
	...
	//MAKING SURE RECEIVER POTTNAME IS GOOD
	if($validatorObject->stringContainsNoTags($input_receiver_pottname) !== true){
...
***********************************************************************************************************/
	...
	$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "SELECT blocked_pottname FROM " . BLOCKED_POTTS_TABLE_NAME . " WHERE sender_pottname = ? AND blocked_pottname = ?", 2, "ss", array($input_receiver_pottname, $input_mypottname));
	if($prepared_statement === false){
		$miscellaneousObject->respondFrontEnd3(3, $languagesObject->getLanguageString("request_failed", $input_language));
	}

	// GETTING RESULTS OF QUERY INTO AN ARRAY
	$prepared_statement_results_array = $preparedStatementObject->getPreparedStatementQueryResults($prepared_statement, array("blocked_pottname"), 1, 1);

	if($prepared_statement_results_array === false){
		$miscellaneousObject->respondFrontEnd3(3, $languagesObject->getLanguageString("request_failed", $input_language));
	}

	$sys_blocked_pottname = trim($prepared_statement_results_array[0]);

	if($sys_blocked_pottname == $input_mypottname){
		$miscellaneousObject->respondFrontEnd3(3, $languagesObject->getLanguageString("request_failed", $input_language));
	} 
	...
	if(trim($prepared_statement_results_array[0]) != "chat_id" && trim($prepared_statement_results_array[0]) != ""){
		...
		$sys_chat_id = $input_chat_id_array[0];
	}

	// GETTING THE NEWS CONTENT
	$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "INSERT INTO " . CHAT_MESSAGES_TABLE_NAME . " (chat_id, sender_pottname, receiver_pottname, message_text, message_time) VALUES (?, ?, ?, ?, ?)" , 5, "sssss", array($sys_chat_id, $input_mypottname, $input_receiver_pottname, $input_message, $input_msg_datetime ));
	...
	$miscellaneousObject->sendNotificationToUser(
		...
		"@" . $input_mypottname . " " . $languagesObject->getLanguageString("sent_you_a_message", $input_language), 
		date("F j, Y"), 
		$input_id
	);
