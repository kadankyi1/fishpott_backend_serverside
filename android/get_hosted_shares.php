...
***********************************************************************************************************/
	...
	//$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "UPDATE " . USER_BIO_TABLE_NAME . " SET coins_secure_datetime = ? WHERE investor_id = ?", 2, "ss", array(date("Y-m-d H:i:s"), $input_id));
		...
		. SHARES_HOSTED_TABLE_NAME . ".parent_company_name FROM " 
		. SHARES_HOSTED_TABLE_NAME . " WHERE " . SHARES_HOSTED_TABLE_NAME . ".flag_not_tradable = 0  ORDER BY  " . SHARES_HOSTED_TABLE_NAME . ".sku DESC";


	$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), $news_fetch_query, 0, "",array());
	...
	$prepared_statement_results_array->bind_result($parent_shares_id, $share_name, $shares_logo, $value_per_share, $yield_duration, $yield_per_share, $company_pottname, $parent_company_name);
	...
	while($prepared_statement_results_array->fetch()){
		...
		$sys_dividend_duration_text = $languagesObject->getLanguageString("pays_dividends_every", $input_language) . " " . $yield_duration . " " . $languagesObject->getLanguageString("days2", $input_language);
