...
***********************************************************************************************************/
	...
	$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "SELECT investor_id FROM " . USER_BIO_TABLE_NAME . " WHERE pot_name = ?", 1, "s", array($input_victim_pottname));
	...
	// GETTING RESULTS OF QUERY INTO AN ARRAY
	$prepared_statement_results_array = $preparedStatementObject->getPreparedStatementQueryResults($prepared_statement, array("block_date"), 1, 1);
	...
	// IF THE DATABASE QUERY GOT NO RESULTS
	if(    trim($prepared_statement_results_array[0]) == "investor_id"
	){
		$miscellaneousObject->respondFrontEnd3(3, $languagesObject->getLanguageString("pott_not_found", $input_language));
	}


	$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "SELECT block_date FROM " . BLOCKED_POTTS_TABLE_NAME . " WHERE sender_pottname = ? AND blocked_pottname = ?", 2, "ss", array($input_mypottname, $input_victim_pottname));
	...
	// IF THE DATABASE QUERY GOT NO RESULTS
	if(    trim($prepared_statement_results_array[0]) == "block_date"
	){
		$block_id = $input_mypottname . "_" . $input_victim_pottname;
		$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "INSERT INTO " . BLOCKED_POTTS_TABLE_NAME . " (block_id, sender_pottname, blocked_pottname, block_date) VALUES (?, ?, ?, ?)" , 4, "ssss", array($block_id, $input_mypottname, $input_victim_pottname, date("Y-m-d H:i:s")));

		if($prepared_statement === false){
			$miscellaneousObject->respondFrontEnd3(3, $languagesObject->getLanguageString("request_failed", $input_language));
		}
		$sys_return_info = $languagesObject->getLanguageString("pott_blocked", $input_language);
	} else {
		$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "DELETE FROM " . BLOCKED_POTTS_TABLE_NAME . " WHERE sender_pottname = ? AND blocked_pottname = ?", 2, "ss", array($input_mypottname, $input_victim_pottname));

		if($prepared_statement === false){
			$miscellaneousObject->respondFrontEnd3(3, $languagesObject->getLanguageString("request_failed", $input_language));
		}
		$sys_return_info = $languagesObject->getLanguageString("pott_unblocked", $input_language);
	}
