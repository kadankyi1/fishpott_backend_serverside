...
if( $_SERVER["REQUEST_METHOD"] == "POST" &&
	...
	isset($_POST["comment_message"]) && trim($_POST["comment_message"]) != "" &&
	...
	$input_comment_message = trim($_POST["comment_message"]);
...
***********************************************************************************************************/
	...
	$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "SELECT profile_picture FROM " . USER_BIO_TABLE_NAME . " WHERE pot_name = ?", 1, "s", array($input_mypottname));
	if($prepared_statement === false){
		$miscellaneousObject->respondFrontEnd3(3, $languagesObject->getLanguageString("request_failed", $input_language));
	}
	...
	// IF THE DATABASE QUERY GOT NO RESULTS
	if(    trim($prepared_statement_results_array[0]) == "profile_picture"
	){
		$miscellaneousObject->respondFrontEnd3(3, $languagesObject->getLanguageString("request_failed", $input_language));
	}

	$sys_receiver_profile_picture = trim($prepared_statement_results_array[0]);
	...
	$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "SELECT inputtor_id FROM " . NEWS_TABLE_NAME . " WHERE news_id = ?", 1, "s", array($input_news_id));

	if($prepared_statement === false){
		$miscellaneousObject->respondFrontEnd3(3, $languagesObject->getLanguageString("request_failed", $input_language));
	}
	// GETTING RESULTS OF QUERY INTO AN ARRAY
	$prepared_statement_results_array = $preparedStatementObject->getPreparedStatementQueryResults($prepared_statement, array("inputtor_id"), 1, 1);

	if($prepared_statement_results_array === false){
		$miscellaneousObject->respondFrontEnd3(3, $languagesObject->getLanguageString("request_failed", $input_language));
	}
	// IF THE DATABASE QUERY GOT NO RESULTS
	if(trim($prepared_statement_results_array[0]) != "inputtor_id" && trim($prepared_statement_results_array[0]) != ""){
		$sys_inputtor_id = $prepared_statement_results_array[0];
	} else {
		$miscellaneousObject->respondFrontEnd3(3, $languagesObject->getLanguageString("news_not_found", $input_language));
	}
	...
	. LOGIN_TABLE_NAME . ".id "
	. " WHERE " . USER_BIO_TABLE_NAME . ".investor_id = ? AND " . LOGIN_TABLE_NAME . ".flag = 0 ORDER BY " .  USER_BIO_TABLE_NAME . ".sku DESC";

	$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), $news_fetch_query, 1, "s", array($sys_inputtor_id));
	...
	$prepared_statement = $preparedStatementObject->prepareAndExecuteStatement($dbObject->connectToDatabase(0, $GLOBALS["USAGE_MODE_IS_LIVE"]), "INSERT INTO " . COMMENTS_TABLE_NAME . " (news_id, inputtor_id, date_time, comment) VALUES (?, ?, ?, ?)" , 4, "ssss", array($input_news_id, $input_id, date("Y-m-d H:i:s"), $input_comment_message));
	...
	$miscellaneousObject->sendNotificationToUser(
		...
		$input_news_id, 
		$input_mypottname , 
		$languagesObject->getLanguageString("new_comment_on_post", $input_language), 
		"@" . $input_mypottname . " " . $languagesObject->getLanguageString("said_something_about_your_post", $input_language), 
		date("F j, Y"), 
		$input_id
	);
