...
if( $_SERVER["REQUEST_METHOD"] == "POST" &&
	...
	isset($_POST["stats_type"]) && trim($_POST["stats_type"]) != "" && intval($_POST["stats_type"]) > -1 &&
...
***********************************************************************************************************/
	...
	if($input_stats_type == 0 || $input_stats_type == 1){
		...
		. LIKES_TABLE_NAME . ".like_type FROM "  
		. USER_BIO_TABLE_NAME . " INNER JOIN "
		. LIKES_TABLE_NAME . " ON  "
		. USER_BIO_TABLE_NAME . ".investor_id=" 
		. LIKES_TABLE_NAME . ".liker_investor_id "
		. " WHERE " . LIKES_TABLE_NAME . ".likes_news_id = ?  AND " . LIKES_TABLE_NAME . ".like_type = $sys_reaction_type ORDER BY " . LIKES_TABLE_NAME .".sku DESC  LIMIT 150";
		...
		. VIEWS_TABLE_NAME . ".views FROM "  
		. USER_BIO_TABLE_NAME . " INNER JOIN "
		. VIEWS_TABLE_NAME . " ON  "
		. USER_BIO_TABLE_NAME . ".investor_id=" 
		. VIEWS_TABLE_NAME . ".viewer_id "
		. " WHERE " . VIEWS_TABLE_NAME . ".news_id = ?  ORDER BY " . VIEWS_TABLE_NAME .".sku DESC  LIMIT 150";
		...
		. PURCHASES_TABLE_NAME . ".item_quantity FROM "  
		. USER_BIO_TABLE_NAME . " INNER JOIN "
		. PURCHASES_TABLE_NAME . " ON  "
		. USER_BIO_TABLE_NAME . ".investor_id=" 
		. PURCHASES_TABLE_NAME . ".buyer_id "
		. " WHERE " . PURCHASES_TABLE_NAME . ".adetor_news_id = ?  ORDER BY " . PURCHASES_TABLE_NAME .".sku DESC  LIMIT 150";
		...
		. COMMENTS_TABLE_NAME . ".comment FROM "  
		. USER_BIO_TABLE_NAME . " INNER JOIN "
		. COMMENTS_TABLE_NAME . " ON  "
		. USER_BIO_TABLE_NAME . ".investor_id=" 
		. COMMENTS_TABLE_NAME . ".inputtor_id "
		. " WHERE " . COMMENTS_TABLE_NAME . ".news_id = ?  ORDER BY " . COMMENTS_TABLE_NAME .".sku DESC  LIMIT 150";

	}

	//echo "news_fetch_query : " . $news_fetch_query;
	...
	$prepared_statement_results_array->bind_result($pot_name, $profile_picture, $date_time, $sys_reaction_type);
	...
	while($prepared_statement_results_array->fetch()){
			...
			$next  = array(				
				"0a" => $input_stats_type, //int setStatsType;
				"1" => $input_news_id, //String setNewsId;
				"2" => strval($sys_reaction_type), //String setReactionOrComment;
				"3" => $newsTime, //String setDate;
				"4" => $pot_name, //String setSenderPottName;
				"5" => $profile_picture //String setSenderPottPic;
				);
