      ...
      $result2 = $mysqli->query($query);
            ...
            if($news_sku == "0" || $gettype == "up"){
                  ...
                  $gettype = "down";
                    ...
                    for ($news_sku; $news_sku > 0; $news_sku--) { 
                        ...
                        $query = "SELECT sku, shares_news_id, parent_shares_id FROM shares4sale WHERE sku = $news_sku";
                          ...
                          if (mysqli_num_rows($result) != "0") {
                              ...
                              $row = $result->fetch_array(MYSQLI_ASSOC);
                              $this_shares_news_id = trim($row["shares_news_id"]);
                              $this_share_sku = trim($row["sku"]);
                              $this_parent_shares_id = trim($row["parent_shares_id"]);

                            $query = "SELECT sku FROM shares_worso WHERE UPPER(share_name) LIKE UPPER('%$search_txt%') OR  UPPER(parent_company_name) LIKE UPPER('%$search_txt%') OR  UPPER(country_origin) LIKE UPPER('%$search_txt%') AND parent_shares_id = '$this_parent_shares_id'";

                              $result = $mysqli->query($query);

                              if (mysqli_num_rows($result) != "0") {

                                    $f_Ids["sku"][$f_Ids_cnt] = $this_share_sku;
                                    $f_Ids["news_id"][$f_Ids_cnt] = $this_shares_news_id;
            ...
            for ($i; $i < $stop_check; $i++) { 
              ...
              if($ads_cnt_for_reset == 2 && count($ads) > $ads_cnt){
                ...
                $query = "SELECT * FROM newsfeed WHERE news_id = '$news_id'  AND flag = 0";
                unset($add_set);
