<?php
  ...
  if(isset($new_news) && $new_news == 1 && isset($new_news_id) && $new_news_id != "") {

    $query = "SELECT * FROM newsfeed WHERE news_id = '$new_news_id' ";

  } elseif (isset($p_inputtor_id) && $p_inputtor_id != "" &&  isset($oldestsku) && $oldestsku != "") {

    $query = "SELECT * FROM newsfeed WHERE sku = $i  AND inputtor_id = '$p_inputtor_id'";

  } else {

    $query = "SELECT * FROM newsfeed WHERE sku = $i ";
  }
