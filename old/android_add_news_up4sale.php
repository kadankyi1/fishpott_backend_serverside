...
if(isset($_POST['myid']) && trim($_POST['myid']) != "" && isset($_POST['mypass']) && trim($_POST['mypass']) != "" && isset($_POST['news_type']) && trim($_POST['news_type']) != "" && trim($_POST['news_type']) == "up4sale" && isset($_POST['inputtor_type']) && trim($_POST['inputtor_type']) != "" && isset($_POST['this_currency']) && trim($_POST['this_currency']) != "" && isset($_POST['item_current_location']) && trim($_POST['item_current_location']) != "" && isset($_POST['item_name']) && trim($_POST['item_name']) != "" && isset($_POST['addNewsText']) && trim($_POST['addNewsText']) != "" && isset($_POST['delivery_style']) && trim($_POST['delivery_style']) != "" && isset($_POST['item_quantity']) && trim($_POST['item_quantity']) != "" && isset($_POST['set_price']) && trim($_POST['set_price']) != "" && isset($_POST['item_weight_type']) && trim($_POST['item_weight_type']) != "") {
...
//$item_current_location = mysqli_real_escape_string($mysqli, $_POST['item_current_location']);
...
$delivery_style = mysqli_real_escape_string($mysqli, $_POST['delivery_style']);
$item_weight_type = mysqli_real_escape_string($mysqli, $_POST['item_weight_type']);
$payment_way = "MM";
    ...
    $item_current_location = trim($item_current_location);
    $pickup_address = strtolower($item_current_location);
    $investor_id = $myid;
	define('KB', 1024);
	define('MB', 1048576);
    ...
    if (mysqli_num_rows($result) != 0) {
          ...
          if($mypass == $dbpass && $flag == 0) {
					...
					if($delivery_style == "Ferry" && substr($pickup_address,0,5) != "ferry"){
    					
						echo "News did not upload. Pickup Address has to be a ferry address";
							exit;
					} else if($delivery_style == "Ferry" && substr($pickup_address,0,5) == "ferry"){
						include(ROOT_PATH . 'inc/db_connect_ferry.php');
							$query = "SELECT add_name FROM addressofmine WHERE add_id = '$pickup_address'";

									$result = $mysqli2->query($query);

									if (mysqli_num_rows($result) == "0") {
										echo "News did not upload. Ferry address not found";
										exit;
									}
					}
...
//////////////////////    FCM  END      /////////////////////////
								...
								$column11_name = "item_weight_type";
								...
								$column11_value = $item_weight_type;
