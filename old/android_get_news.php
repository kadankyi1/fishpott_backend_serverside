...
if(isset($_POST['myid']) && trim($_POST['myid']) != "" && isset($_POST['mypass']) && trim($_POST['mypass']) != "" && isset($_POST['news_sku']) && trim($_POST['news_sku']) != "" && isset($_POST['gettype']) && trim($_POST['gettype']) != "" && isset($_POST['i_country']) && trim($_POST['i_country']) != "") {
    ...
    if (mysqli_num_rows($result) != 0) {
          ...
          if($mypass == $dbpass) {
            ...
            $ads = array();
        		...
        		$query="SELECT blocked_id FROM mern_ha_me_fuo WHERE sender_id = '$myid'";
            	$result = $mysqli->query($query);

            	while($row=$result->fetch_array()) {
                   
                    $this_blocked_id = $row["blocked_id"];
			        array_push($blockedPotts, $this_blocked_id);

                }

                $news_fetch_query_ext = " ";

                if(count($blockedPotts) > 0){

                	for ($i=0; $i < count($blockedPotts); $i++) { 

$news_fetch_query_ext = $news_fetch_query_ext . " AND inputtor_id != '" . $blockedPotts[$i] . "' ";
...
//////////////////////    FCM  END      /////////////////////////			            
      ...
      $result2 = $mysqli->query($query);
          	...
          	if($news_sku == "0" || $gettype == "up"){
		    	...
		    	if($skip_add == 0 && $latest_sku == $news_sku){
    				echo json_encode($newsfeedReturn); exit;
		    	}
		    	if($skip_add == 1){
				...
				$gettype = "down";
          	    ...
          	    if($gettype == "down"){
    				...
    				for ($i; $i > 0; $i--) { 
    					...
    					if($i == $news_sku_real && $news_sku_real != ""){
    						break;
    					}
    					...
    					if($ads_cnt_for_reset == 2 && count($ads) > $ads_cnt){
    						...
    						$add_set = 1;
    						$ads_cnt = $ads_cnt + 1;
    						$query = "SELECT * FROM newsfeed WHERE news_id = '$ad_news_id'  AND flag = 0 " . $news_fetch_query_ext;

    					} else {

    						$ads_cnt_for_reset = $ads_cnt_for_reset + 1;
    						$query = "SELECT * FROM newsfeed WHERE sku = $i AND flag = 0 " . $news_fetch_query_ext;
    						//echo "query --- " . $query;
    						unset($add_set);
						  ...
						  if (mysqli_num_rows($result) != "0") {
						      ...
						      $news_views = intval($row["news_views"]);
            ...
            preg_match_all('#\bhttps?://[^,\s()<>]+(?:\([\w\d]+\)|([^,[:punct:]\s]|/))#', $addNewsText, $match);
				...
				//echo "host 333 -- " . $r["host"] . "<br>";
					          ...
					          $query = "SELECT like_type FROM likes WHERE likes_news_id = '$news_id' AND liker_investor_id = '$investor_id' ";
					          $result = $mysqli->query($query);
					          if (mysqli_num_rows($result) != "0") {

					                $row = $result->fetch_array(MYSQLI_ASSOC);
					                $db_like_type = intval($row["like_type"]);
		                ...
		                $up4sale_item_location =  $output["results"][1]["formatted_address"];
								...
								if (mysqli_num_rows($result) != "0") {
							          ...
							          if (trim($sn_news_video) == "" || !file_exists("../user/" . $sn_news_video)) {
										...
										$query = "SELECT COUNT(*) FROM comments WHERE news_id = '$news_id_ref'";   


										$result = $mysqli->query($query);
										$row = $result->fetch_array(MYSQLI_ASSOC);
										$sn_num_of_comments = $row["COUNT(*)"];

										if(!isset($sn_num_of_comments)  || trim($sn_num_of_comments) == "") {

										$sn_num_of_comments == "0"; 
...
$news_sub = $share_name . ". Originating Country : " . $country_origin . ".  Quantity On Sale: " . $num_on_sale . ". Price Per Share : " . $new_amt_user_str . ". Yields " . $yield_per_share . " total for every " . $yield_duration . " days";
							...
							if ($item_1 != "sku" && $item_1 != "" && $item_2 != "news_id" && $item_2 != ""){
							  ...
							  $item_2 = "news_id";
								...
								include(ROOT_PATH . 'inc/db_connect_autologout.php'); 
						        $sql = "SELECT COUNT(*) FROM adetor WHERE adetor_news_id = '$news_id'";
								  $result = $mysqli->query($sql);

								  $row = $result->fetch_array(MYSQLI_ASSOC);
								  $p_num_of_buys = $row["COUNT(*)"];
...
/****** APP CHALLENGE CHANGE END*************/
		                      ...
		                      'shared_news_likes_num_REAL' => $sn_p_num_of_likes,
		                      'shared_news_dislikes_num_REAL' => $sn_p_num_of_dislikes,
		                      'shared_news_views' => $sn_news_views,         
		                      'shared_news_comments' => $sn_num_of_comments,            
